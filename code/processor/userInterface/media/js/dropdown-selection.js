...
$(document).ready(function(){
  ...
  if(fiveW_onload==''){ 
      var this_col2_options = {'Please select in order': ''};
       ...
       var this_col2_options = col2_options[qtype][fiveW_onload];
        $.each(this_col2_options, function(key, value) {
          $('#id_col2').append($("<option></option>")
               .attr("value", value).text(key));
               });
          $('#id_col2').val(col2_onload);
         var this_col_options = last_col_options[qtype][fiveW_onload][col2_onload];
         $.each(this_col_options, function(key, value) {
           $('#id_last_col').append($('<option></option>')
               .attr('value', value).text(key));
            });
...
$('#id_fiveW').change(function(e){
  ...
  //alert(fiveW_val);
    ...
    var this_col2_options = col2_options[qtype][fiveW_val];
    $.each(this_col2_options, function(key, value) {
      $('#id_col2').append($("<option></option>")
           .attr("value", value).text(key));
           });
...
$('#id_col2').change(function(e){
  ...
  //alert(fiveW_val);
    ...
    var this_col_options = last_col_options[qtype][fiveW_val][col2_val];
    $.each(this_col_options, function(key, value) {
      $('#id_last_col').append($("<option></option>")
           .attr("value", value).text(key));
           });
